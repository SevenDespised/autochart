...
class Processor(IProcessor):
    ...
    def generate_prompt(self, input_data, data: StageExecutionData = None) -> str:
        ...
        """
        # 获取输入数据中的查询文本
        query = input_data["nl_queries"]
        # 获取schema信息
        db_name = input_data["db_name"]
        db_path = os.path.join(DB_DIR, db_name)
        # 获取db_path下所有table
        tables_path = [os.path.join(db_path, f) for f in os.listdir(db_path) if f.endswith('.csv')]
        schema_info = get_csv_schema(tables_path)
        # 获取hint信息
        describe = input_data.get("describe")
        describe = describe if describe else "None Describe"
        ir_table = str(input_data.get("irrelevant_tables"))
        ...
        # 优化提示词
        self._prompt_optimizer = PromptOptimizer(query, 'en')
